...
function STARTPROGRAM() //use this to start the program (autogenerates the array of Part Prefixes which can then be ran through the sorting method) *******ONLY RUN THIS TO START THE PROGRAM*******
{
  ...
  */
  ...
  sheet = activeSpreadsheet.getSheetByName("Summary");
  sheet.appendRow([' ', ' ', ' ']);
  sheet.appendRow(["Total Sales", totalQuan, "$" + totalSales]);
}
...
function DeleteSheets(){ //Delete all existing sheets besides the MRP ******UPDATE TO BE AUTOMATED******
  ...
  var sheets = ["ES003", "ES004" , "ES005", "ES010","ES012","ES013", "ES014", "ES015", "ES016","ES020", "ES021", "ES011", "Extra", "ES002", "Summary"]; 
...
function subtotals(name){ //will display sub-totals for each individual product's sheets, add quantity from each indivudal three digit ending of product ID
  ...
  sheet.appendRow(["7/09/2018", "Epiq Solutions", "Ryan Regier", count, count])
}
...
function format(name){ //formats header at top of each sheet (bolds it and freezes it)
  ...
  if(name === "Extra")
  {
    ...
    s.getRange(1, 1).setFontWeight("bold");
    s.getRange(2, 1).setFontWeight("bold");
    s.getRange(2, 2).setFontWeight("bold");
    s.getRange(2, 3).setFontWeight("bold");
  }
  else
  {
    s.getRange(1, 1).setFontWeight("bold");
...
function CreateSummary(){
  moveSheets_ascending()
  var activeSpreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var yourNewSheet = activeSpreadsheet.getSheetByName("From_MRP");
  var lastRow = yourNewSheet.getLastRow();
  var data = yourNewSheet.getDataRange().getValues();
  var activeSpreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var yourNewSheet = activeSpreadsheet.getSheetByName("Summary");
  if (yourNewSheet != null) {
    activeSpreadsheet.deleteSheet(yourNewSheet);
  }
  yourNewSheet = activeSpreadsheet.insertSheet();
  yourNewSheet.setName("Summary");
  yourNewSheet.appendRow(['Date range:',data[1][0], data[lastRow-1][0]]);
  yourNewSheet.appendRow(['Part No', 'Quantity', 'Sales']);
  format("Summary");
}


function SummaryFill(name){
  //name = "ES002";
  var activeSpreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = activeSpreadsheet.getSheetByName(name);
  var data = sheet.getDataRange().getValues();
  ...
  activeSpreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  sheet = activeSpreadsheet.getSheetByName("Summary");
  sheet.appendRow([name, data[lastRow-1][3], "$" + data[lastRow-1][5]]);
  totalSales += data[lastRow-1][5];
  totalQuan += data[lastRow-1][3];
}
