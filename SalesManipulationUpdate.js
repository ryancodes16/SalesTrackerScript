...
function salesTab(){ //new features for the script
  Logger.clear();
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  range.setValue("$" + QBdata[1][1].valueOf());
  Logger.log(QBdata[1][1].valueOf());
  
  var range2 = s.getRange("E" + temp);
  var numSold = range2.getValue();
  Logger.log(numSold + " sold");
  range = s.getRange("G" + temp); 
  range.setValue("$" + (QBdata[1][1].valueOf() / numSold));
...
function salesTab1(){ //updated salesTab to work for multiple sales for different parts inputted into quickbooks
  ...
  for(var z = 1; z < QBdata.length; z++){
    var name = QBdata[z][0].valueOf();
    Logger.log(name);
    if(name === "Extra")
    {
      var num = 0;
      s = ss.getSheetByName("Extra");
      partData = s.getDataRange().getValues();
      for(var i = 1; i < partData.length; i++){
        num += partData[i][3].valueOf();
      }
      range = s.getRange("E" + i);
      range.setValue("$" + (QBdata[z][1].valueOf() / num));    
    }
    else
    {
      Logger.log(name.substring(0,5));
      ...
      Logger.log(temp);
      range = s.getRange("F" + temp); 
      range.setValue("$" + QBdata[z][1].valueOf());
      Logger.log(QBdata[z][1].valueOf());
      
      var range2 = s.getRange("E" + temp);
      var numSold = range2.getValue();
      Logger.log(numSold + " sold");
      range = s.getRange("G" + temp); 
      range.setValue("$" + (QBdata[z][1].valueOf() / numSold));
    }
  }
}
